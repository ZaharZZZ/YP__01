...
namespace TestAdmissionCommittee
{
    ...
    public class TStorageApplicant
    {
        ...
        public void AddApplicant_WhenDbInsertFails_ShouldReturnFalse(int id, string fullName)
        {
            // Arrange
            var mockDbManager = new Mock<IApplicantDBManager>();
            var storageApplicant = new StorageApplicant(mockDbManager.Object);
            ...
            // Настройка мока - абитуриент не существует, но вставка не удалась
            mockDbManager.Setup(x => x.CheckApplicantExists(id)).Returns(false);
            mockDbManager.Setup(x => x.InsertApplicant(It.IsAny<Applicant>())).Returns(false);
            ...
            Assert.IsFalse(result);
            mockDbManager.Verify(x => x.CheckApplicantExists(id), Times.Once);
            mockDbManager.Verify(x => x.InsertApplicant(It.IsAny<Applicant>()), Times.Once);
        }
        ...
        [DataRow(2)]
        [DataRow(3)]
        public void AddApplicant_WithNullValues_ShouldHandleCorrectly(int id)
        {
            // Arrange
            var mockDbManager = new Mock<IApplicantDBManager>();
            var storageApplicant = new StorageApplicant(mockDbManager.Object);
